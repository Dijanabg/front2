...
const Example1 = () => {
  ...
  return (
    <div>
      ...
      <button
        onClick={() => {
          let result = fetch("http://localhost:8080/api/v1/book/1");
          result
            .then((val) => {
              if (val.ok) {
                return val.json();
...
const Example2 = () => {
  ...
  return (
    <div>
      ...
      <button
        onClick={async () => {
          let result = await fetch("http://localhost:8080/api/v1/book/5");
          if (result.ok) {
...
const Example4 = () => {
  ...
  useEffect(() => {
    ...
    const ff = async () => {
      //B
      let r = await fetch("http://localhost:8080/api/v1/genre/1");
      let rr = await r.json();
      if (!ignore) {
        setData(rr);
      }
...
//http://localhost:8080/ednevnik/ocene/ucenik/1
const Example5 = () => {
  const [data, setData] = useState(null);
  return (
    <div>
      ...
      <button
        onClick={async () => {
          let result = await fetch("http://localhost:8080/api/v1/users");
          if (result.ok) {
            let d = await result.json();
            setData(d);
          } else {
